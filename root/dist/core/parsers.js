...
export async function parseCity(text, context, logger) {
    ...
    if (nlpCandidate.success && (nlpCandidate.confidence ?? 0) >= CITY_NLP_MIN && nlpCandidate.data?.normalized) {
        if (logger?.debug)
            logger.debug({ nlpCandidate }, 'city_nlp_accepted');
        return nlpCandidate;
    }
    // LLM fallback
    try {
        ...
        if (logger?.debug && nlpCandidate.success)
            logger.debug({ nlpCandidate, llm: result }, 'city_llm_override_nlp');
        return {
            success: true,
            data: result,
        ...
        for (const rx of heuristicPatterns) {
            ...
            if (typeof cand === 'string' && cand.length > 0 && !TEMPORAL_WORDS.includes(cand.toLowerCase())) {
                ...
                if (!/^[A-Z][A-Za-z\- ]+$/.test(normalized) || stoplist.has(normalized.toLowerCase()))
                    continue;
                if (logger?.debug)
                    logger.debug({ cand: normalized }, 'city_heuristic_last_resort');
                return {
                    success: true,
 ...
 */
function parseCityWithNLP(text) {
    try {
        ...
        const tokensAny = doc.tokens().out();
        const stoplist = new Set(['hey', 'hi', 'hello', 'thanks', 'thank you', 'ok', 'okay', 'what', 'where', 'when', 'why', 'how', 'which', 'who']);
 ...
 */
export async function parseDate(text, context, logger) {
    ...
    if (process.env.USE_COMPROMISE_DATES === 'true') {
        try {
            const nlpMod = (await import('compromise')).default;
            const datesMod = (await import('compromise-dates')).default;
            const n = nlpMod.extend(datesMod);
            const doc = n(text);
            const found = doc?.dates()?.get?.()?.[0]?.text || '';
            if (found) {
                if (logger?.debug)
                    logger.debug({ found }, 'date_compromise_accepted');
                return {
                    success: true,
    ...
    if (nlpCandidate.success && (nlpCandidate.confidence ?? 0) >= DATE_NLP_MIN && nlpCandidate.data?.dates) {
        if (logger?.debug)
            logger.debug({ nlpCandidate }, 'date_nlp_accepted');
        return nlpCandidate;
    }
    // LLM fallback
    try {
        ...
        if (logger?.debug && nlpCandidate.success)
            logger.debug({ nlpCandidate, llm: result }, 'date_llm_override_nlp');
        return {
            success: true,
            data: result,
            confidence: result.confidence,
 ...
 */
function parseDateWithNLP(text) {
    try {
        const doc = nlp.readDoc(text);
...
Return only valid JSON.`;
        ...
        if (logger?.debug && nlpCandidate.success)
            logger.debug({ nlpCandidate, llm: result }, 'od_llm_override_nlp');
        if (result.confidence > 0.5) {
            return {
                success: true,
                data: result,
 ...
 */
function parseOriginDestinationWithNLP(text) {
    try {
        const doc = nlp.readDoc(text);
        const tokens = doc.tokens();
        const tokenArray = tokens.out();
 ...
 */
export async function parseIntent(text, context, logger) {
 ...
 */
function parseIntentWithPatterns(text, context) {
 ...
 */
export async function extractSlots(text, context, logger) {
    const slots = {};
