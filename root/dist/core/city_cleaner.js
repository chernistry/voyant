...
export async function cleanCityName(rawCity, log) {
    ...
    const parsed = await parseCity(rawCity, {}, log).catch(() => ({ success: false }));
    if (parsed && parsed.success && parsed.data?.normalized)
        return parsed.data.normalized.trim();
...
export async function extractCleanCity(text, log) {
    ...
    const parsed = await parseCity(text, {}, log).catch(() => ({ success: false }));
    if (parsed && parsed.success && parsed.data?.normalized)
        return parsed.data.normalized.trim();
