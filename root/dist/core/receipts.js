...
export const FactSchema = z.object({
    source: z
        .enum(['Open-Meteo', 'REST Countries', 'Wikipedia', 'OpenTripMap', 'Brave Search'])
        .or(z.string()),
    key: z.string(),
    value: z.any(),
...
export function buildReceiptsSkeleton(facts, decisions, token_estimate) {
    const sources = Array.from(new Set(facts.map((f) => f.source)));
    const ext_api_latency_ms = facts.reduce((sum, f) => sum + (typeof f.latency_ms === 'number' ? f.latency_ms : 0), 0);
    return {
        sources,
        decisions,
