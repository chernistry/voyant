...
export async function performDeepResearch(query, context = {}, log) {
    ...
    const domainFirstUrl = new Map();
    for (const item of top) {
        try {
            const d = new URL(item.url).hostname;
            if (!domainFirstUrl.has(d))
                domainFirstUrl.set(d, item.url);
        }
        catch { }
    }
    const domains = Array.from(domainFirstUrl.keys());
    const citations = domains.map((d) => ({ source: d, url: domainFirstUrl.get(d) || '', confidence: 0.7 }));
...
async function optimizeQueries(original, context, log) {
...
function deduplicateByDomain(items) {
    const seen = new Set();
    const out = [];
