...
async function decideShouldSearch(message, ctx) {
...
async function detectQueryType(message, ctx) {
        ...
        if (input.route.intent === 'weather') {
            ...
            try {
                ...
                if (destinationFacts.length > 0) {
                    ...
                    const destinations = destinationFacts.map(f => `${f.value.city}, ${f.value.country} (${f.value.tags.climate}, ${f.value.tags.budget} budget)`).join('; ');
...
function extractMonthOrDates(text) {
    const m = text.match(/\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\b/i) ??
        text.match(/\d{4}-\d{2}-\d{2}\s*\.\.\s*\d{4}-\d{2}-\d{2}/);
    return m?.[0];
}
function parseTemps(summary) {
    const m = summary.match(/High\s+(-?\d+(?:\.\d+)?)°C\s*\/\s*Low\s+(-?\d+(?:\.\d+)?)°C/i);
    if (!m)
        return undefined;
...
function chooseBandFromTemps(maxC, minC) {
    if (typeof maxC === 'number' && maxC >= 26)
        return 'hot';
    if (typeof minC === 'number' && minC <= 5)
        return 'cold';
    if (typeof maxC === 'number' || typeof minC === 'number')
        return 'mild';
    return undefined;
}
