...
function scrubString(input) {
    ...
    out = out.replace(/\b\d{4}-\d{2}-\d{2}\s*\.\.\s*\d{4}-\d{2}-\d{2}\b/g, '[REDACTED_DATES]');
    // Replace ISO dates (YYYY-MM-DD)
    out = out.replace(/\b\d{4}-\d{2}-\d{2}\b/g, '[REDACTED_DATE]');
    // Replace month names (Jan..Dec)
    out = out.replace(/\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\b/gi, '[REDACTED_MONTH]');
 ...
 */
export function scrubPII(arg, enabled) {
    if (!enabled)
        return arg;
 ...
 */
export function scrubMessage(msg, enabled) {
