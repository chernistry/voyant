...
export function getBreaker(host) {
    if (!breakers.has(host)) {
        const config = getConfig(host);
        const breaker = new CircuitBreaker(async (fn) => fn(), config);
        initStats(host);
...
export async function withBreaker(host, fn) {
    const breaker = getBreaker(host);
    return breaker.fire(fn);
}
export function getBreakerStats(host) {
