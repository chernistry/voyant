...
export async function searchPOIs(input) {
    ...
    const kinds = input.kinds ?? 'interesting_places,sights,monuments,museums,architecture';
    const url = `${BASE_URL}/radius?lon=${encodeURIComponent(String(input.lon))}` +
    ...
    try {
        ...
        if (geo.success) {
            const pois = geo.data.features.map((f) => ({
        ...
        if (j.success) {
            const pois = j.data.features.map((f) => ({
                xid: f.xid,
                name: f.name || '',
                kinds: f.kinds,
