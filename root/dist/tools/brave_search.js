...
export async function searchTravelInfo(query, log, deepResearch = false) {
    ...
    if (!apiKey) {
        if (log)
            log.debug(`❌ Brave Search: no API key configured`);
    ...
    try {
        ...
        if (log)
            log.debug(`🔗 Brave Search: using wrapper for query`);
        ...
        if (log)
            log.debug(`✅ Brave Search success after ${duration}ms`);
        ...
        if (e instanceof Error) {
            ...
            if (errorMessage.includes('rate limit') || errorMessage.includes('429')) {
                if (log)
                    log.debug(`🚫 Brave Search rate limited`);
                return { ok: false, reason: 'rate_limited' };
            }
            if (errorMessage.includes('unauthorized') || errorMessage.includes('401') || errorMessage.includes('403')) {
                if (log)
                    log.debug(`🔑 Brave Search authentication error`);
                return { ok: false, reason: 'auth_error' };
            }
            if (errorMessage.includes('timeout')) {
                if (log)
                    log.debug(`⏰ Brave Search timeout`);
                return { ok: false, reason: 'timeout' };
            }
            if (errorMessage.includes('network') || errorMessage.includes('fetch')) {
                if (log)
                    log.debug(`🌐 Brave Search network error`);
                return { ok: false, reason: 'network' };
            }
        }
        if (log)
            log.debug(`❓ Brave Search unknown error type: ${e?.constructor?.name || typeof e}`);
 ...
 */
export function extractWeatherFromResults(results, city) {
 ...
 */
export function extractCountryFromResults(results, country) {
 ...
 */
export function extractAttractionsFromResults(results, city) {
