    ...
    for (let attempt = 0; attempt < delays.length; attempt++) {
        try {
            ...
            if (attempt > 0) {
                ...
                if (log)
                    log.debug(`⏳ Rate limit backoff: waiting ${delay}ms before attempt ${attempt + 1}...`);
                await new Promise(resolve => setTimeout(resolve, delay));
            }
            const url = `https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(query)}&count=7`;
            if (log)
                log.debug(`🔗 Brave Search URL (attempt ${attempt + 1}): ${url}`);
            ...
            if (log)
                log.debug(`✅ Brave Search success on attempt ${attempt + 1}`);
            ...
            if (log)
                log.debug(`❌ Brave Search error (attempt ${attempt + 1}/${delays.length}):`, e);
            ...
            if (attempt === delays.length - 1) {
                if (log)
                    log.debug(`❌ Brave Search failed after ${delays.length} attempts due to rate limiting`);
