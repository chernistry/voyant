...
export async function recommendDestinations(slots) {
    try {
        ...
        const scored = filtered.map(c => {
            let score = 0;
            if (isFamily && c.family)
                score += 2;
            if (slots.budget && c.budget === slots.budget)
                score += 1;
            if (slots.climate && c.climate === slots.climate)
                score += 1;
            // Boost popular destinations slightly
            if (['Paris', 'London', 'Rome', 'Barcelona', 'Amsterdam'].includes(c.city))
                score += 0.5;
            return { c, score };
